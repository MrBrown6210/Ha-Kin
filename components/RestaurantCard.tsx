...
import X from "../public/star-empty.svg";
...
const RestaurantCard: FunctionComponent<RestaurantProps> = ({ restaurant }) => {
  return (
    <div>
      ...
      <div>
        <div className="mt-2 text-lg leading-6 font-semibold">
          {restaurant.title}
        </div>
        <div className="flex mt-1">
          {Array(restaurant.stars).fill(
            <svg
        ...
        <div className="flex space-x-1">
          {Array(restaurant.averageCost).fill(<div>$</div>)}
        </div>
      </div>
    </div>
  );
